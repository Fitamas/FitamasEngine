...
namespace WDL.Gameplay.DigitalLogic
{
    public struct Component
    {
        ...
        public LogicComponentDescription Description;

        public bool NotNull => Id > 0 || Description != null;
    ...
    {
        public string Name = "LogicComponent";
        ...
        public bool IsBase => ComponentType != typeof(LogicCircuit);
        public bool IsOutput => typeof(IOutputLogicComponent).IsAssignableFrom(ComponentType);
        public bool IsINPUT => ComponentType == typeof(LogicINPUT);
        public bool IsOUTPUT => ComponentType == typeof(LogicOUTPUT);

        public string FullName
        {
            get
            {
                return Path.Combine(Namespace, Name);
            }
        }
        ...
        public LogicComponentDescription() : this(typeof(LogicCircuit))
        {

        }

        public LogicComponentDescription(Type componentType) 
        {
            Namespace = "";
            ComponentType = componentType;
        }

        public Component GetFromIndex(int index)
        {
        ...
        public void Remove(LogicComponentDescription description)
        {
            for (int i = 0; i < Components.Count; i++)
            {
                if (Components[i].Description == description)
                {
                    Components.RemoveAt(i);
                }
            }
